...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/05/06 15:34:05 by alganoun          #+#    #+#             */
/*   Updated: 2022/05/06 18:08:59 by alganoun         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
    ...
    while (server > 0) 
    {
        ...
        */
        ...
        do {
			...
			client_++;
        } while (client_ != 0 || buffer[strlen(buffer) - 1] != '\n');
		client_ = -1;
        do {
			std::stringstream ss;
            cout << "\nServer: ";
            do {
                cin >> buffer;
				ss << buffer << std::endl;
                send(server, ss.str().c_str(), bufsize, 0);
                if (*buffer == '#') {
                    send(server, buffer, bufsize, 0);
                    *buffer = '*';
                    isExit = true;
                }
            } while (ss.str()[ss.str().length() - 1] != '\n');       		cout << "Client: ";
        	do {
           		recv(server, buffer, bufsize, 0);
            	cout << buffer << " ";
            	if (*buffer == '#') {
			...
			client_++;
       		} while (client_ != 1 || buffer[strlen(buffer) - 1] != '\n');
			client_ = -1;
        } while (!isExit);
